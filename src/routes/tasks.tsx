...
import { EMPLOYEES, type AppTask, type TaskPriority, type TaskStatus } from "@/data/seed";
...
function TasksPage() {
  ...
  const visible = useMemo(() => {
    ...
    if (scope === "mine") list = list.filter((t) => t.assigneeId === actor.id);
      ...
      const e = employees.find((x) => x.id === t.assigneeId);
  ...
  }, [tasks, scope, actor.id, actor.team, employees]);
      ...
      {draftOpen && (
        <NewTaskModal
          ...
          onCreate={(input) => {
            ...
            toast.success(`Task assigned to ${EMPLOYEES.find((e) => e.id === t.assigneeId)?.name.split(" ")[0]}`, {
